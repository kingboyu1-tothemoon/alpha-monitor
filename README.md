...
- 美股代码：例如 `NVDA`、`TSLA`、`AAPL`
- A 股 6 位代码：例如 `600519`、`300750`、`000001`
- A 股中文名称或简称：例如 `贵州茅台`、`茅台`、`宁德`、`比亚迪`
...
美股使用 Reddit / X / Google Trends 作为情绪平台标签；A 股使用东方财富股吧 / 雪球 / 百度指数作为情绪平台标签。当前版本没有接入这些平台的真实 API，因此这些不是实时提及量或真实搜索指数，而是免费数据下的情绪扩散近似判断。

## 数据源

后端接口 `/api/capital-flow` 当前使用 Nasdaq 的无 key 延迟历史行情接口处理美股，使用东方财富免费延迟行情处理 A 股。
