...
搜索股票后，系统会评估社交情绪是否开始扩散：

- Reddit 热度：用相对成交量、单日波动和成交额做散户讨论热度代理
- X 提及量：用短线涨跌、5 日趋势、产业和财报催化做传播速度代理
- Google Trends：用 20 日趋势、产业景气度和财报事件强度做搜索兴趣代理

当前版本没有接入 Reddit API、X API 或 Google Trends API，因此这些不是实时提及量或真实搜索指数，而是免费数据下的情绪扩散近似判断。
